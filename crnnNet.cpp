...
bool crnnNet::forward(cv::Mat& src, TextLine& result) {
    ...
    int wpad = (resized_w + 31) / 32 * 32 - resized_w;

    cv::Size tmp = cv::Size(resized_w + wpad, dstHeight);

    ncnn::Mat input =
        ncnn::Mat::from_pixels_resize(src.data, ncnn::Mat::PIXEL_BGR2RGB,
...
crnnNet::crnnNet() { net.opt.use_vulkan_compute = true; }